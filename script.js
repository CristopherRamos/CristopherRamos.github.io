...
const revealObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if(entry.isIntersecting){
      ...
      // se for um número que deve "contar" (ex: 100%), anima ele subindo do 0
...
// conta de 0 até o número em data-target (ex: 100), em ~1.2s, e
// mantém o sufixo (data-suffix, ex: "%") no final do texto.
function animateCounter(el){
  ...
  function tick(now){
    ...
    const eased = 1 - Math.pow(1 - progress, 3); // easeOutCubic, desacelera no final
